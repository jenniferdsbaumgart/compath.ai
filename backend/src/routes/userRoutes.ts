...
// Login user
router.post("/login", validateRequest(userSchemas.login), async (req, res) => {
  try {
...
router.post(
  "/coins/spend",
  authenticateToken,
  validateRequest(userSchemas.spendCoins),
  async (req: any, res) => {
    try {
...
router.post("/profile", authenticateToken, async (req: any, res) => {
  try {
    ...
    if (!questionId || response === undefined) {
      return res
        .status(400)
        .json({ message: "Question ID and response are required" });
    }
  ...
  async (req: any, res) => {
      ...
      const profiles = [
        {
          profile: "Social",
          description:
            "Enjoys community engagement, helping others, and promoting well-being.",
        },
        {
          profile: "Analítico",
          description:
            "Organized, detail-oriented, and excels in problem-solving and logistics.",
        },
        {
          profile: "Criativo",
          description:
            "Innovative, artistic, and enjoys creating unique products or experiences.",
        },
        {
          profile: "Comunicador",
          description:
            "Dynamic, expressive, and skilled at engaging and serving audiences.",
        },
        {
          profile: "Técnico",
          description:
            "Skilled in technical tasks, technology, and optimizing systems.",
        },
      ];

      const prompt = `
      Based on the following user responses, classify their entrepreneurial profile into one of these categories: ${profiles
        .map((p) => p.profile)
        .join(", ")}.
    ...
    `;
      ...
      const selectedProfile =
        completion.choices[0].message.content?.trim() || "Social";
