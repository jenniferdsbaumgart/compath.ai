...
function normalizeMetrics(resp: any): DashboardMetrics {
  const m = resp?.metrics ?? resp ?? {};
  return {
    coins: Number(m.coins) || 0,
    searchCount: Number(m.searchCount) || 0,
    activeCourses: Array.isArray(m.activeCourses) ? m.activeCourses : [],
    totalUsers: Number(m.totalUsers) || 0,
    totalCourses: Number(m.totalCourses) || 0,
    totalSearches: Number(m.totalSearches) || 0,
    profileCompletion: Number(m.profileCompletion) || 0,
    userActivity: {
      invitedFriends: Number(m?.userActivity?.invitedFriends) || 0,
    },
  };
}
...
export default function DashboardPage() {
  ...
  const [user, setUser] = useState<{ name: string; coins: number } | null>(
    null
  );
      ...
      try {
        ...
        const [userCoins, dashboardResponse] = await Promise.all([
          ...
          api.getDashboardData(currentUser.id).catch((err) => {
            ...
            return {}; // voltaremos com um objeto vazio e deixamos o normalizador fazer o resto
          }),
        ]);

        const safeMetrics = normalizeMetrics(dashboardResponse);
      ...
      setDashboardData((prev) =>
        prev
          ? {
              ...
              userActivity: {
                ...prev.userActivity,
                invitedFriends: prev.userActivity.invitedFriends + 1,
              },
            }
          : prev
      );
  ...
  const metrics = dashboardData ?? normalizeMetrics({});
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-300 mt-1">
                Bem-vindo ao seu dashboard da plataforma Compath
              </p>
            </div>
        ...
        <section className="mb-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Responda algumas perguntas para que possamos recomendar os
                melhores nichos para você
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Progress
              ...
              <div className="mt-2 flex flex-col sm:flex-row sm:justify-between">
                <div className="flex items-center text-sm text-gray-600">
                  ...
                  <span className="dark:text-gray-400">
                    Ganhe 100 moedas ao completar seu perfil
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Link href="/cursos">
              <Card className="cursor-pointer hover:shadow-md transition-shadow duration-200">
                ...
                <CardContent>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Veja suas pesquisas favoritas salvas para acessar
            ...
            <div className="bg-white dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {metrics.userActivity.invitedFriends}
                  </p>
                </div>
                  ...
                  <Card key={course.id}>
                    ...
                    <CardContent>
                      ...
                      <p className="text-sm mt-2">
                        Custo:{" "}
                        {course.coinCost !== undefined
                          ? `${course.coinCost} moedas`
                          : "Gratuito"}
                      </p>
                      {course.duration && (
                        <p className="text-sm">Duração: {course.duration}</p>
                      )}
                    </CardContent>
                  </Card>
                ))}
